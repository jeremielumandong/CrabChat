...
pub fn render(frame: &mut Frame, state: &AppState) {
    ...
    let browser = &state.search_results;

    let title = format!(
        " Search Results \u{2014} {} ({} results) ",
        browser.title,
    ...
    let filter_line = Line::from(vec![
        ...
        Span::styled("\u{2588}", Style::default().fg(Theme::ACCENT_TEAL)),
    ]);
    ...
    let header = Line::from(Span::styled(
        format!(
            ...
            "Size",
            bot_w = bot_col_w,
            file_w = (inner.width as usize).saturating_sub(bot_col_w + size_col_w + 6),
            size_w = size_col_w,
        ),
        Style::default()
    ...
    if browser.filtered.is_empty() {
        ...
        let file_col_w = (inner.width as usize).saturating_sub(bot_col_w + size_col_w + 6);
        ...
        for vis_i in start..end {
            ...
            // Truncate filename safely at char boundaries
            let filename_display: String = {
                let chars: Vec<char> = item.filename.chars().collect();
                if file_col_w < 4 {
                    String::new()
            ...
            // Truncate bot name
            let bot_display: String = {
                let chars: Vec<char> = item.bot.chars().collect();
                if chars.len() > bot_col_w {
                    chars[..bot_col_w.saturating_sub(3)]
                        .iter()
                        .collect::<String>()
                        + "..."
                } else {
    ...
    let help = Line::from(vec![
        ...
        Span::styled(" Request  ", Style::default().fg(Theme::TEXT_SECONDARY)),
        Span::styled(
            "Esc",
            Style::default()
                .fg(Theme::ACCENT_AMBER)
